...
monty_hall_game <- function(door_num = 3, switch = FALSE) {
        ...
        if (switch) {
                ...
                door_cross_out <- ifelse(door_price == initial_selection,
                                         base::sample(doors[!doors %in% c(door_price, initial_selection)], door_num - 2),
                                         doors[!doors %in% c(door_price, initial_selection)])
...
ggplot(df, aes(x = switch, y = count, fill = result)) +
        ...
        labs(title = "Month Hall Problem Simulation Result", 
